package org.crimsoncode2026.notifications.permissions
...
class NotificationPermissionHandler(
    ...
    fun isNotificationPermissionGranted(): Boolean {
        return permissionsController.isPermissionGranted(Permission.POST_NOTIFICATIONS)
    }
    ...
    fun notificationPermissionState(): Flow<NotificationPermissionState> {
        return permissionsController.observePermission(Permission.POST_NOTIFICATIONS)
            .map { state ->
                when {
    ...
    suspend fun requestNotificationPermission(): Boolean {
        return try {
            permissionsController.providePermission(Permission.POST_NOTIFICATIONS)
